; woldvein Trainer v0.2 Installer Script for Inno Setup 6
; 生成 woldvein_trainer_v0.2_setup.exe 安装包
;
; 特性：
;   - 中英双语（安装时选择语言）
;   - 自由选择安装路径
...
#define MyAppURL "https://github.com/woldvein/woldvein-trainer"
#define MyAppExeName "woldvein_trainer_v0.2.exe"
...
; 输出配置
OutputDir=..\
OutputBaseFilename=woldvein_trainer_v0.2_setup
...
UninstallDisplayName=woldvein Trainer v0.2
...
[Files]
; 主程序和 DLL（必装）
Source: "dist\woldvein_trainer_v0.2.exe"; DestDir: "{app}"; Flags: ignoreversion noregerror
...
begin
  ...
  if Exec(ExpandConstant('{cmd}'), '/C taskkill /F /IM woldvein_trainer_v0.2.exe >nul 2>nul', '', SW_HIDE, ewWaitUntilTerminated, ResultCode) then
  begin
    // 已尝试关闭，继续安装
  end;
  Result := True;
