# 平野孤鸿 全能修改器 woldvein Trainer v0.2

> 专为西山居城建经营游戏《平野孤鸿》(BalladsOfHongye, Steam AppID 2656540) 开发的游戏修改工具。
>
> 纯内存操作 · DLL 注入 · Lua 执行引擎 · 6 大功能标签页 · 11 个全局热键 · 深色主题 GUI
...
| 层级 | 技术 | 说明 |
|------|------|------|
| GUI 层 | Python 3 + tkinter + ttk | 深色主题，6 大标签页 |
...
修改器共 **6 大功能标签页**，覆盖从基础资源修改到高级游戏控制的完整功能矩阵。

### 1. 资源修改

支持 **9 种基础资源** 的一键修改和自定义数量增加（游戏后续升级可能新增更多资源）：
...
1. **下载**：获取 `woldvein_trainer_v0.2.exe` 和 `woldvein_trainer.dll`，放在同一目录
2. **启动游戏**：通过 Steam 启动《平野孤鸿》
3. **启动修改器**：右键 `woldvein_trainer_v0.2.exe` → 以管理员身份运行（热键需要）
...
> 注：v0.2 早期版本的"散件覆盖+定时重置"方案已废弃，直接 hook 阻止消耗更简洁且无 UI 异常。
...
> 注：v0.2 早期版本曾 hook `GetBuildingCards` / `GetTypedBuildingCards` 过滤非最高级建筑，但完全重写这两个方法会破坏 UI 数据结构导致游戏 UI 全部消失，该实现已移除。
...
│   └── woldvein_trainer_v0.2.exe    # 打包版 EXE（PyInstaller onefile）
│
...
│   ├── resource_editor.py   # 资源修改（9种资源 + 幸福度 + 知名度）
...
| v0.2 | Inline-Hook `lua_pcall` | 全覆盖，当前版本 |
...
pyinstaller --onefile --windowed --name "woldvein_trainer_v0.2" `
...
```
dist/
├── woldvein_trainer_v0.2.exe    # 主程序（约 30-40 MB）
└── woldvein_trainer.dll         # DLL（可外置也可打包进 EXE）
```
...
3. **首次启动慢**：onefile 模式首次启动需要解压到临时目录，约 2-5 秒
...
*README 版本：v0.2 详细版 | 更新日期：2026-09-12*
